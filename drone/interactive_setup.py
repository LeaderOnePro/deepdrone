...
PROVIDERS = {
    ...
    "Qwen": {
        ...
        "description": "Qwen3.5 models via DashScope (OpenAI-compatible)",
    },
    "xAI": {
        "name": "xai",
        "models": [
...
def install_ollama_model(
    ...
    try:
        ...
        console.print(
            f"[yellow]📥 Installing {model_name} on {base_url}... This may take a few minutes.[/yellow]"
        )
...
def get_model_name(provider_name: str, provider_config: Dict) -> Optional[str]:
    ...
    if provider_name.lower() == "ollama":
        ...
        console.print(
            "  • [green]Local[/green]: [cyan]http://localhost:11434[/cyan] (default)"
        )
        console.print(
            "  • [blue]LAN[/blue]: [cyan]http://192.168.1.100:11434[/cyan] (replace with your server IP)"
        )
        console.print(
            "  • [blue]Internet[/blue]: [cyan]https://your-domain.com:11434[/cyan] (if exposed to internet)"
        )
        console.print(
            "\n[dim]Make sure Ollama server is running with: ollama serve[/dim]"
        )
        console.print(
            "[dim]For network access, start with: OLLAMA_HOST=0.0.0.0 ollama serve[/dim]\n"
        )
        ...
        choice = Prompt.ask("Select option or enter custom URL", default="1")
        ...
        if choice == "1" or choice.lower() == "local":
            ...
            custom_url = Prompt.ask(
                "Enter custom Ollama server URL", default="http://localhost:11434"
            )
            ollama_base_url = (
                custom_url.strip()
            ...
            ollama_base_url = "http://localhost:11434"
            console.print(
                f"[yellow]Invalid input, using default: {ollama_base_url}[/yellow]"
            )
        ...
        if local_models:
            ...
            for i, model in enumerate(provider_config["models"], start_idx):
                console.print(
                    f"  {i}. [blue]{model}[/blue] [dim](will be downloaded)[/dim]"
                )
            ...
            if ollama_base_url == "http://localhost:11434":
                console.print(
                    "Make sure Ollama is running locally: [cyan]ollama serve[/cyan]"
                )
            else:
                console.print(
                    f"Make sure Ollama server is running on: [cyan]{ollama_base_url}[/cyan]"
                )
                console.print(
                    "For network access, start with: [cyan]OLLAMA_HOST=0.0.0.0 ollama serve[/cyan]"
                )
            console.print()
        ...
        try:
            ...
            result = Prompt.ask(
                "Enter model name or number from list above", default="1"
            )
            ...
            if result:
                ...
                try:
                    ...
                    if 1 <= choice_num <= len(all_options):
                        ...
                        if selected_model not in local_models:
                            console.print(
                                f"[yellow]Model '{selected_model}' not found locally.[/yellow]"
                            )
                            from rich.prompt import Confirm

                            if Confirm.ask(
                                f"Would you like to install {selected_model}?",
                                default=True,
                            ):
                                if install_ollama_model(
                                    selected_model, ollama_base_url
                                ):
                                ...
                                console.print(
                                    "[yellow]Model installation cancelled[/yellow]"
                                )
                                return None
                ...
                if model_name not in local_models:
                    console.print(
                        f"[yellow]Model '{model_name}' not found locally.[/yellow]"
                    )
                    from rich.prompt import Confirm

                    if Confirm.ask(
                        f"Would you like to install {model_name}?", default=True
                    ):
...
def get_api_key(provider_name: str, model_name: str) -> Optional[str]:
    ...
    if provider_name.lower() == "zhipuai":
        console.print(
            "[yellow]格式提示：请填写以 id.secret 形式的 API Key（示例：abc123.def456）。[/yellow]"
        )
        console.print("[dim]该 Key 将用于生成 JWT 以调用 ZhipuAI API。[/dim]\n")
    elif provider_name.lower() == "qwen":
        console.print(
            "[yellow]提示：Qwen 使用 OpenAI 兼容通道，请填入 DashScope 的 API Key。[/yellow]\n"
        )
    elif provider_name.lower() == "minimax":
        console.print(
            "[yellow]提示：MiniMax 使用 OpenAI 兼容通道，请填入 MiniMax 的接口密钥。[/yellow]\n"
        )
    ...
    try:
        ...
        if api_key and api_key.strip():
            if provider_name.lower() == "zhipuai" and "." not in api_key.strip():
                console.print(
                    "[red]❌ 无效的 ZhipuAI API Key，必须为 id.secret 形式[/red]"
                )
                return None
...
def start_interactive_session():
    ...
    try:
        ...
        if provider_name.lower() == "ollama":
            ...
            console.print(
                "[green]✅ Ollama runs locally - no API key required![/green]"
            )
        ...
        console.print(
            "  • [green]Simulator[/green]: [cyan]tcp:127.0.0.1:5762[/cyan] (default)"
        )
        console.print(
            "  • [blue]Real Drone USB[/blue]: [cyan]/dev/ttyACM0[/cyan] (Linux) or [cyan]COM3[/cyan] (Windows)"
        )
        console.print(
            "  • [blue]Real Drone TCP[/blue]: [cyan]tcp:192.168.1.100:5760[/cyan]"
        )
        console.print(
            "  • [blue]Real Drone UDP[/blue]: [cyan]udp:192.168.1.100:14550[/cyan]\n"
        )

        from rich.prompt import Prompt

        connection_string = Prompt.ask(
            "Enter drone connection string", default=default_connection
        )

        if not connection_string:
            console.print(
                "[yellow]Using default connection: tcp:127.0.0.1:5762[/yellow]"
            )
        ...
        # Step 6: Start chat
        console.print(
            "[bold green]🚀 Starting DeepDrone chat session...[/bold green]\n"
        )
